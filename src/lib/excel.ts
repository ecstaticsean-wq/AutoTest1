...
type FieldKey = "productName" | "salesUrl";
...
export async function parseProductExcel(file: File): Promise<ParsedProductExcel> {
  ...
  for (let rowNumber = 2; rowNumber <= sheet.rowCount; rowNumber++) {
    ...
    const salesUrl = cellToText(row.getCell(columnByField.get("salesUrl")!).value);

    if (!productName && !salesUrl) continue;

    if (!isNonEmptyProductName(productName)) {
      rowErrors.push({ rowNumber, message: "상품명이 비어 있습니다." });
      continue;
    }
    if (!isValidUrl(salesUrl)) {
      rowErrors.push({ rowNumber, message: "판매URL 형식이 올바르지 않습니다." });
      continue;
    }

    rows.push({ rowNumber, productName, salesUrl });
  }
...
export async function downloadProductExcelTemplate(): Promise<void> {
  ...
  sheet.columns = [
    ...
    { header: HEADER_LABELS.salesUrl, key: "salesUrl", width: 48 },
  ];
  sheet.addRow({ productName: "제주 한라봉 5kg", salesUrl: "https://example.com/hallabong" });
...
export async function downloadRowsAsExcel(
  rows: { productName: string; salesUrl: string }[],
  filename: string
): Promise<void> {
  const workbook = new ExcelJS.Workbook();
  const sheet = workbook.addWorksheet("상품목록");
